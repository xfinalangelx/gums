...
const ManageAds = () => {
  ...
  const [totalAdsList, setTotalAdsList] = useState([]);
  ...
  // Submit ManageAds form
  async function submitManageAdsForm() {
    if (documentValidator) {
      const { error, data } = await supabase.from("ads").insert({}).select();
  ...
  async function deleteAd(file, id) {
    const { data, error } = await supabase.storage.from("ads").remove([file]);
    ...
    <>
      ...
      <div>
            ...
            <div className="bg-white rounded-md overflow-auto py-8 px-10 w-[600px]">
              <div className="w-full flex flex-col items-center justify-center gap-4">
                <p className="font-semibold text-3xl text-center pt-1">
                  Upload new advertisement picture (Make sure image is 600px x
                  700px) and in PNG format
                </p>
        ...
        <table className="w-full min-w-max table-auto text-left">
          ...
          <tbody>
            {totalAdsList.map(({ id, created_at }, index) => {
              const isLast = index === totalAdsList.length - 1;
